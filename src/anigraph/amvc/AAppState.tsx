...
export abstract class AAppState<NodeModelType extends ASceneNodeModel, SceneModelType extends ASceneModel<NodeModelType>> extends AObject {
  ...
  @AObjectState dataSet: { [key: string]: number; };
  @AObjectState uploadingData: boolean;
  @AObjectState biggerIsBetter: boolean;
  ...
  async dataSetUploaded(file: File) {
    ...
    for (const line of lines) {
      if (line.length !== 0) {
        ...
        const valueTrimmed = value.includes('\r') ? value.substring(0, value.length - 1) : value;
        const valueAsNum = parseFloat(valueTrimmed)
        this.dataSet[name] = valueAsNum;
